...
interface AdoptionFormData {
  ...
  how_heard?: string;
  which_animal?: string; // Animal they are interested in (can be pre-filled or entered)
  // --- TODO: Add other adoption-specific questions here ---
  // Example:
  // current_pets?: string; // Description of current pets
  // previous_pets?: string; // Description of previous pets
  // vet_info?: string; // Veterinarian contact info
  // home_type?: 'House' | 'Apartment' | 'Condo' | 'Other';
  // yard_type?: 'Fenced' | 'Unfenced' | 'None';
  // --- End of TODO ---
  subject: string; // Hidden/Internal
...
export default function AdoptionForm({ animalName, animalId, onClose }: AdoptionFormProps) {
  ...
  const defaultSubject = animalName
    ? `Adoption Application for ${animalName} (ID: ${animalId})` // Specific subject
    : `Adoption Application`; // Generic subject

  const defaultWhichAnimal = animalName ? `${animalName}` : ""; // Pre-fill only if name is provided
  ...
  const {
    ...
    mode: "onTouched",
    // Use the conditional default values
    defaultValues: {
      subject: defaultSubject,
      which_animal: defaultWhichAnimal,
      primary_phone_type: '', // Sensible defaults
  ...
  const onSubmit = async (data: AdoptionFormData) => {
    ...
    try {
      ...
      // Prepare payload, potentially adding original animal context if needed by web3forms/email template
      const payload = {
        ...data,
        access_key: process.env.NEXT_PUBLIC_WEB3FORMS_ACCESS_KEY,
        from_name: "Rescue App Adoption Application",
        // Conditionally add original context if useful for backend/email
        ...(animalName && { inquiry_for_animal_name: animalName }),
        ...(animalId && { inquiry_for_animal_id: animalId }),
        botcheck: undefined, // Don't send honeypot
      }
  ...
  // --- FORM JSX (Apply consistent styling) ---
  return (
      ...
      <div className="flex-grow p-6 bg-gray-50 dark:bg-gray-800 overflow-y-auto">
        {!isSuccess ? (
          <form onSubmit={handleSubmit(onSubmit)} noValidate>
            ...
            <h4 className="text-lg font-medium mb-3 text-gray-800 dark:text-gray-200">Your Information</h4>
			...
			{/* --- Adoption Specific Section --- */}
            <hr className="my-6 border-gray-300 dark:border-gray-600" />
            <h4 className="text-lg font-medium mb-3 text-gray-800 dark:text-gray-200">Adoption Information</h4>

            {/* Which Animal */}
            <div className="mb-4">
              <label htmlFor="which_animal" className="block mb-2 text-sm text-gray-600 dark:text-gray-400">
                Which animal(s) are you interested in? { !animalName && "(Optional)" }
              </label>
              <textarea
                id="which_animal"
                rows={2} // Can be short if pre-filled
                {...register("which_animal")} // Make required only if !animalName? Optional for now.
                placeholder={!animalName ? "Enter name(s) or type(s) of animal(s) you're interested in" : ""}
                className={`w-full p-2 border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:ring focus:ring-indigo-100 dark:focus:ring-indigo-900 focus:border-indigo-500 dark:focus:border-indigo-500`}
              />
              {/* No error message needed if optional, add if required */}
            </div>
            ...
            <div className="flex justify-end gap-3 pt-4 border-t border-gray-200 dark:border-gray-700 mt-6">
              ...
              <button
                ...
                className="px-4 py-2 text-white bg-indigo-500 rounded-md hover:bg-indigo-600 focus:bg-indigo-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 dark:focus:ring-offset-gray-800 disabled:opacity-50" // Indigo theme
              >
