...
export const Spotlights = (props: Readonly<SpotlightsProps>) => {
  ...
  if (!data.desc || data.desc.length <= maxLength) {
      // If description is short or null, display it as-is
      descriptionContent = (
      ...
      // If it's too long, truncate and add "Read More"
    ...
    <> {/* Use Fragment to wrap component and modal */}
      <Container className="flex flex-wrap mb-20 lg:gap-10 lg:flex-nowrap ">
          ...
          <div
            ...
            <Link
              href={`/animal/${data.id}`}
              className="relative w-full aspect-w-1 aspect-h-1 lg:aspect-w-16 lg:aspect-h-9"
            >
              <Image
                src={typeof data.image === 'string' ? data.image : data.image.src}
                alt={data.name || 'Spotlight Animal'}
                fill
          ...
          <div
            className={`flex flex-wrap items-center w-full lg:w-2/5 ${imagePosition === "right" ? "lg:justify-end lg:pl-10" : "lg:pr-10"
              }`}
          >
            <div>
              <div className="flex flex-col w-full mt-4">
                ...
                {/* Use the descriptionContent variable */}
                {descriptionContent}
              </div>
