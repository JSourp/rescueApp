...
interface VolunteerFormData {
  ...
  // Volunteer Specific
  areas_of_interest?: string[]; // For checkboxes
  other_skills?: string; // Textarea
  location_acknowledgement: boolean | string; // Could be boolean or specific string value
  volunteer_reason?: string; // Optional text/select/radio
...
export default function VolunteerForm({ onClose }: VolunteerFormProps) {
  ...
  const {
    ...
    defaultValues: {
      ...
      location_acknowledgement: false, // Default to not acknowledged
      areas_of_interest: [], // Default for checkboxes
    }
  });
  ...
  const onSubmit = async (data: VolunteerFormData) => {
    ...
    console.log("Volunteer Form Data:", data);
     ...
     // Basic check for location acknowledgement - adjust based on how you implement it
     if (data.location_acknowledgement !== true && data.location_acknowledgement !== 'accepted') {
         setSubmitMessage("Please acknowledge the location information to proceed.");
         return;
     }
    ...
    try {
      ...
      const payload = {
          ...
          areas_of_interest: data.areas_of_interest?.join(', '), // Convert array to string
      ...
      if (result.success) {
        setIsSuccess(true);
        setSubmitMessage("Thank you! Your volunteer application has been submitted. Someone from our team will follow up with you shortly.");
            ...
            {!isSuccess ? (
                <form onSubmit={handleSubmit(onSubmit)} noValidate>
                    ...
                    {/* --- Volunteer Specific Section --- */}
                     ...
                     <h4 className="text-lg font-medium mb-3 text-gray-800 dark:text-gray-200">Volunteer Information</h4>

                     {/* Areas of Interest (Checkboxes) */}
                    <div className="mb-4">
                        <label className="block mb-2 text-sm text-gray-600 dark:text-gray-400">Areas of Interest (Check all that apply)</label>
                        <div className="space-y-2">
                            {['Dog Walking', 'Cat Care / Socialization', 'Shelter Cleaning / Support', 'Adoption Events', 'Fundraising', 'Transport', 'Admin / Office Help'].map((interest) => (
                            <div key={interest} className="flex items-center">
                                <input
                                type="checkbox"
                                id={`interest-${interest.replace(/\s+/g, '-')}`} // Create unique ID
                                value={interest}
                                {...register("areas_of_interest")} // Register array field
                                className="h-4 w-4 text-orange-600 border-gray-300 dark:border-gray-600 rounded focus:ring-orange-500 dark:bg-gray-700"
                                />
                                <label htmlFor={`interest-${interest.replace(/\s+/g, '-')}`} className="ml-2 text-sm text-gray-700 dark:text-gray-300">{interest}</label>
                            </div>
                            ))}
                        </div>
                     </div>
                     ...
                     {/* Other Skills */}
                    <div className="mb-4">
                        <label htmlFor="other_skills" className="block mb-2 text-sm text-gray-600 dark:text-gray-400">Other Skills or Talents (e.g., marketing, training, photography, graphic design)</label>
                        <textarea
                            id="other_skills"
                            rows={3}
                    ...
                    <div className="mb-4">
                        <label htmlFor="volunteer_reason" className="block mb-1 text-sm text-gray-600 dark:text-gray-400">Reason for volunteering? (Optional)</label>
                         <select id="volunteer_reason" {...register("volunteer_reason")} className={`w-full p-2 border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring focus:ring-orange-100 dark:focus:ring-orange-900 focus:border-orange-500 dark:focus:border-orange-500`}>
                 ...
                 // Success Message Area (Adapt from AnimalInquiryForm/PopupWidget)
