...
namespace rescueApp
{
	public class GetFosterApplications
	{
		private readonly AppDbContext _dbContext;
		private readonly ILogger<GetFosterApplications> _logger;
		...
		public GetFosterApplications(AppDbContext dbContext, ILogger<GetFosterApplications> logger)
		{
		...
		[Function("GetFosterApplications")]
		public async Task<HttpResponseData> Run(
			// Security is handled by internal Auth0 Bearer token validation and role-based authorization.
			[HttpTrigger(AuthorizationLevel.Anonymous, "GET", Route = "foster-applications")]
			AzureFuncHttp.HttpRequestData req)
		{
			_logger.LogInformation("C# HTTP trigger function processed GetFosterApplications request.");
			...
			try
			{
				...
				IQueryable<FosterApplication> query = _dbContext.FosterApplications
					.Include(app => app.ReviewedByUser); // Include user who reviewed it

				// Apply Filters
				if (!string.IsNullOrWhiteSpace(statusFilter))
				{
					_logger.LogInformation("Filtering foster applications by status: {Status}", statusFilter);
				...
				var applicationsDto = await query
					.Select(app => new FosterApplicationListItemDto
					{
						Id = app.Id,
				...
				_logger.LogInformation("Returning {Count} foster applications.", applicationsDto.Count);
				...
				try
				{
					...
					_logger.LogInformation("Successfully serialized and sent {Count} foster applications.", applicationsDto.Count);
				}
				catch (JsonException jsonEx)
				{
					_logger.LogError(jsonEx, "JSON SERIALIZATION FAILED for foster applications. Count: {Count}", applicationsDto.Count);
				...
				{
					_logger.LogError(writeEx, "Error writing JSON payload to response for foster applications.");
					response = req.CreateResponse(HttpStatusCode.InternalServerError);
			...
			{
				_logger.LogError(ex, "Error fetching foster applications.");
				return await CreateErrorResponse(req, HttpStatusCode.InternalServerError, "An error occurred while fetching foster applications.");
			}
		}
