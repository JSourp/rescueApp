...
namespace rescueApp
{
	...
	public class CreateDocumentMetadataRequest
	{
		...
		public string? DocumentType { get; set; } // e.g., "Vaccination Record"
		...
		public string? BlobName { get; set; } // Unique name generated by previous step

		[Required(AllowEmptyStrings = false)]
		public string? BlobUrl { get; set; } // Base URL generated by previous step
	...
	public class CreateDocumentMetadata
	{
			...
			int animalId) // Animal ID from route
		{
			...
			try
			{
				...
				{
					animal_id = animalId,
					document_type = metadataRequest.DocumentType!,
					file_name = metadataRequest.FileName!,
					blob_name = metadataRequest.BlobName!,
					blob_url = metadataRequest.BlobUrl!,
					description = metadataRequest.Description, // Nullable
					date_uploaded = utcNow, // Let DB default handle? Explicit is fine too.
					uploaded_by_user_id = currentUser.id // Link to logged-in user
				};
                ...
                {
                    newDocument.id,
                    newDocument.animal_id,
                    newDocument.document_type,
                    newDocument.file_name,
                    newDocument.blob_name,
                    newDocument.blob_url,
                    newDocument.description,
                    newDocument.date_uploaded,
                    newDocument.uploaded_by_user_id
			...
			{
				_logger.LogError(dbEx, "Database error saving document metadata for Animal ID: {AnimalId}. BlobName: {BlobName}, InnerEx: {InnerMessage}",
					animalId, metadataRequest.BlobName, dbEx.InnerException?.Message);
			...
			{
				_logger.LogError(ex, "Error saving document metadata for Animal ID: {AnimalId}. BlobName: {BlobName}", animalId, metadataRequest.BlobName);
